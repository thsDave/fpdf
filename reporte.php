...
// Cabecera de página
	...
	function Header() {
		...
		$this->Image('img/logo.png', 10, 10, 40); //imagen(archivo, png/jpg || x,y,tamaño)
		$this->setXY(60, 15);
		$this->Cell(100, 8, 'Reporte de usuarios registrados', 0, 1, 'C', 0);
		$this->Ln(20);
	}
...
// --------------------METODO PARA ADAPTAR LAS CELDAS------------------------------
	...
	function CheckPageBreak($h, $setX) {
		...
		if ($this->GetY() + $h > $this->PageBreakTrigger) {
			...
			$this->Cell(45, 8, 'Email', 1, 0, 'C', 0);
			$this->Cell(30, 8, 'Cargo', 1, 0, 'C', 0);
			$this->Cell(30, 8, 'Region', 1, 0, 'C', 0);
...
$pdf->Cell(45, 8, 'Email', 1, 0, 'C', 0);
$pdf->Cell(30, 8, 'Cargo', 1, 0, 'C', 0);
$pdf->Cell(30, 8, 'Region', 1, 0, 'C', 0);
...
for ($i = 0; $i < count($data); $i++) {
	...
	$pdf->Row(array($i + 1, utf8_decode($data[$i]['name']), utf8_decode($data[$i]['email']), utf8_decode($data[$i]['position']), utf8_decode($data[$i]['region']), utf8_decode($data[$i]['status'])), 8);
}
